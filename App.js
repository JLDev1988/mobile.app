...
export default function App() { 
  const [url, setUrl] = useState('https://www.buyoriginal.pk/'); 
  ...
  registerNNPushToken(8222, 'YxbidQvmJVVSJYDs5fE3zb'); 
  ...
  useEffect(() => { 
    async function getUnreadNots() { 
      let unreadCount = await getUnreadNotificationInboxCount(8222, 'YxbidQvmJVVSJYDs5fE3zb'); 
  ...
  const handleAnalytics = (path) => { 
    axios.post(`https://app.nativenotify.com/api/analytics`, { 
      app_id: 8222, 
      app_token: 'YxbidQvmJVVSJYDs5fE3zb', 
      screenName: path 
    }); 
  }; 
  
  const handleGoToInbox = async () => { 
    let notifications = await getNotificationInbox(8222, 'YxbidQvmJVVSJYDs5fE3zb'); 
